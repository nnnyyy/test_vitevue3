...
const onBtnGoLv4 = (key:string, isOver:boolean)=>{    
    ...
    router.push(`${parent}/${isOver ? state.lv3MouseOver?.key : current?.key}/${key}`).then(()=>{
        ...
        cartStore.addToCart(route.path)
    })
...
<div class="d-flex" style="height: 100%;">
    <div v-if="state.menuLv3" style="width: 180px; border-right: 1px solid #CCCCCC;">
        <nav>
            <div v-for="it in state.menuLv3" class="menu-item d-flex justify-center align-center" style="height: 46px;" @mouseover="showLv4List(it.key)" :class="[isSelected(it.key, 3)?'selected':'', ( showLv4 && state.lv3MouseOver?.key == it.key ) ? 'over' : '']">{{it.name}}</div>
        </nav>
    </div>
    <div style="position:relative; width: 180px; border-right: 1px solid #CCCCCC;">
        <nav>
            <div v-for="it in state.menuLv4" style="height: 46px;" class="menu-item d-flex justify-center align-center" :class="[isSelected(it.key, 4)?'selected':'']" @click="onBtnGoLv4(it.key, false)">{{it.name}}</div>     
        </nav>
        <div v-show="showLv4" style="position:absolute; top: 0; left: 0; width: 180px; height: 100%; border-right: 1px solid #CCCCCC; background-color: gray;" @mouseleave="showLv4=false;">
            <nav>
                <div v-for="it in state.menuLv4MouseOver" style="height: 46px;" class="menu-item d-flex justify-center align-center" @click="onBtnGoLv4(it.key, true)">{{it.name}}</div>     
            </nav>
        </div>
    </div>
